export interface PoliticasEntity {
  id: string;
  type: 'politicas'; // Tipo de entidad
...
/**
 * Implementación funcional e inmutable para Politicas
 * Sigue principios de diseño orientado a objetos y patrones funcionales
 */
export class Politicas implements PoliticasEntity {
  readonly id: string;
  readonly type: 'politicas';
  ...
  private constructor(props: PoliticasEntity) {
    this.id = props.id;
  ...
  static create(data: Partial<PoliticasEntity>): Politicas {
  ...
  withStatus(status: 'PENDING' | 'COMPLETED' | 'CANCELED' | 'REJECTED'): Politicas {
    return new Politicas({
      ...this,
      status,
      updatedAt: new Date(),
  ...
  withNotes(notes: string): Politicas {
    return new Politicas({
      ...this,
      notes,
      updatedAt: new Date()
